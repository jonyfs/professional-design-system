...
test.describe("Radio", () => {
  test.beforeEach(async ({ page }) => {
    await page.goto("/src/components/radio/radio.html");
  });
  ...
  test("focus-visible state matches visual baseline", async ({ page }) => {
    ...
    await expect(page.getByTestId("radio-group")).toHaveScreenshot(
      "radio-focus-visible.png",
    );
  });

  test("disabled option matches visual baseline", async ({ page }) => {
  ...
  test("long label wraps without breaking input/label alignment (Edge Case)", async ({
    ...
    // The input stays at a fixed narrow width regardless of label wrap.
