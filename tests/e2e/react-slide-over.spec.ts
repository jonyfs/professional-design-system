...
const HARNESS_URL = "http://localhost:5174/slide-over.html";

test.describe("Slide-over (React package, visual parity with static reference)", () => {
  ...
  test("closed state matches the static reference's visual baseline", async ({ page }) => {
    await expect(page.getByTestId("slide-over-trigger")).toHaveScreenshot(
      "slide-over-closed.png",
    );
  });

  test("open state matches the static reference's visual baseline (AC1)", async ({ page }) => {
    await page.getByTestId("slide-over-trigger").click();
    await expect(page.getByTestId("slide-over")).toBeVisible();
  ...
  test("clicking the backdrop closes the panel and returns focus to the trigger (AC2)", async ({
    ...
    const panelBox = await page
      .getByTestId("slide-over")
      .locator(".slide-over-panel")
      .boundingBox();
  ...
  test("empty-content variant matches the static reference's visual baseline", async ({
    page,
  }) => {
    await page.getByTestId("slide-over-info-trigger").click();
    await expect(page.getByTestId("slide-over-info")).toBeVisible();
    await expect(page.getByTestId("slide-over-info")).toHaveScreenshot(
      "slide-over-info-open.png",
    );
  });
