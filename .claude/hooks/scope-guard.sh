...
SPEC_FILE=$(find "$PROJECT_ROOT" -path "*/specs/*/spec.md" -mmin -60 -type f -printf '%T@ %p\n' 2>/dev/null \
  | sort -rn | head -1 | cut -d' ' -f2-)

# Fallback for systems without -printf (macOS)
if [ -z "$SPEC_FILE" ]; then
  SPEC_FILE=$(find "$PROJECT_ROOT" -path "*/specs/*/spec.md" -mmin -60 -type f -print0 2>/dev/null \
...
while IFS= read -r FILE; do
  ...
  if [ "$MATCH" -eq 0 ]; then
    ...
    case "$BASENAME" in
      spec.md|plan.md|tasks.md|*.spec.md|*.test.*|*Test.*|*Tests.*|*_test.*|test_*) continue ;;
