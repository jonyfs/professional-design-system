...
across User Stories 2-3 once User Story 1 proves the pipeline.
...
| G6 | VII — Autonomous Skill Acquisition | `design-sync` skill consulted directly (its own `SKILL.md`) to ground technical requirements, not guessed | PASS — this plan's requirements (dist/, .d.ts, compiled CSS) are read from `non-storybook/SKILL.md`, cited in research.md |
...
shared/design-tokens.ts          # NEW — single source of truth for color/radius tokens,
                                  # imported by both tailwind.config.ts (root) and
                                  # packages/react/tailwind.config.ts (prevents drift)
