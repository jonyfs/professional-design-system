...
- [ ] **STILL NOT DONE**: the remaining 14 of the original 20 flagged components (not explicit banned-pattern matches, but still < 4/10): alert, loading-overlay, toast, navbar, sidebar, tabs, list, chart (caveat — stub demo page by design), gallery, pick-list, compare, notification-center, social-login. Lower-confidence/lower-severity than the 6 banned-pattern matches already fixed — genuine follow-up work, not silently dropped.
- [X] T014 — re-ran affected E2E specs (624/624 passed) and regenerated only the genuinely-shifted snapshots (card + dashboard-example; the other 4 touched components have no pixel-snapshot assertions to regenerate) for the 6 components refined in the follow-up session above. Not yet done for the remaining 14.
