...
- [X] **Follow-up session (2026-07-21): the 6 explicit BANNED-pattern matches refined** — card, dashboard-example, data-table/table (shared CSS), button, select, theme-gallery. Dispatched as 6 parallel subagents, each constrained to this catalog's existing semantic tokens only (verified correct across all 119 themes via `audit:contrast`). Two real defects caught in post-agent verification and fixed: card's new badge measured 6.87:1 (below AAA 7:1, switched to a solid-contrast treatment), and dashboard-example's identity-bar text measured 6.86:1 plus a required Divider (spec.md SC-003) was accidentally dropped in that agent's rewrite — both fixed. Full local Playwright run across all 6 touched spec files: 624/624 passed; Linux CI baselines regenerated via `update-snapshots.yml` workflow_dispatch. Delivered as a separate PR (specs/044's own scope, not a new feature).
- [X] **Follow-up session (2026-07-21, remaining-14 pass): all remaining 13 non-banned-pattern flagged components refined** (chart intentionally left untouched — stub demo page by design): alert, loading-overlay, toast, navbar, sidebar, tabs, list, gallery, pick-list, compare, notification-center, social-login. Dispatched as 4 parallel subagents, each constrained to existing semantic tokens (verified across all 119 themes via `audit:contrast`). Two real defects caught in post-agent verification and fixed: gallery-thumb's `@apply group` broke the Tailwind v3 build (moved `group` to markup), and sidebar's `rounded-xl` violated the borderRadius token allowlist (corrected to `rounded-lg`).
- [X] T014 — re-ran affected E2E specs across all 13 components (798 tests) and regenerated the genuinely-shifted snapshots (alert/toast/list/sidebar/tabs + react-toast); functional-only components (loading-overlay/gallery/pick-list/compare/notification-center/social-login/navbar) had no pixel-snapshot drift needing regeneration.
...
- [X] T017 Ran `npm run verify` as a final combined check (typecheck → token audit → contrast audit → build → full E2E suite) — 6067 passed, 31 skipped, 0 failed
- [X] T018 Zero test-coverage reduction confirmed — no test file deletions, only snapshot regeneration + new/expanded coverage
- [X] T019 [P] Ran `graphify update .` to sync the knowledge graph after all changes land
