...
| G3 | IV — Design Token Discipline | Zero raw palette classes (`bg-blue-600`, etc.) in shipped markup | PASS — Badge ring colors also corrected from raw `ring-green-600/20`-style classes to the ratified `ring-success/20`-style tokens during the same remediation. Enforced by `scripts/audit-tokens.mjs`. |
